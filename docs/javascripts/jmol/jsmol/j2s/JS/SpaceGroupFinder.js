...
var bcsame = JS.SpaceGroupFinder.approx0 (b - c);
if (bcsame) c = b * 1.5;
var absame = JS.SpaceGroupFinder.approx0 (a - b);
if (absame) b = a * 1.2;
var acsame = JS.SpaceGroupFinder.approx0 (c - a);
if (acsame) c = a * 1.1;
var alpha = params[3];
var beta = params[4];
var gamma = params[5];
if (JS.SpaceGroupFinder.approx0 (alpha - 90)) {
alpha = 80;
}if (JS.SpaceGroupFinder.approx0 (beta - 90)) {
beta = 100;
}if (JS.SpaceGroupFinder.approx0 (gamma - 90)) {
gamma = 110;
}if (alpha > beta) {
var d = alpha;
alpha = beta;
beta = d;
}var albesame = JS.SpaceGroupFinder.approx0 (alpha - beta);
var begasame = JS.SpaceGroupFinder.approx0 (beta - gamma);
var algasame = JS.SpaceGroupFinder.approx0 (gamma - alpha);
if (albesame) {
beta = alpha * 1.2;
}if (begasame) {
gamma = beta * 1.3;
...
gamma = alpha * 1.4;
}if (JS.SpaceGroupFinder.isHexagonal (n, null)) {
b = a;
if (sg.axisChoice == 'r' ? JU.SimpleUnitCell.isRhombohedral (params) : JS.SpaceGroupFinder.isHexagonal (-1, params)) {
} else if (sg.axisChoice == 'r') {
c = b = a;
if (alpha > 85 && alpha < 95) alpha = 80;
gamma = beta = alpha;
} else {
alpha = beta = 90;
gamma = 120;
...
}sym =  new JS.Symmetry ().setUnitCell ( Clazz.newFloatArray (-1, [a, b, c, alpha, beta, gamma]), false);
...
}}} else if (absame && bcsame && JS.SpaceGroupFinder.approx0 (params[3] - params[4]) && JS.SpaceGroupFinder.approx0 (params[4] - params[5])) {
isRhombo = true;
...
return (JS.SpaceGroupFinder.approx0 (finv - i) ? i : 0);
}, "~N");
